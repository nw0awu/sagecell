...
class KernelProvider(object):
    ...
    def ready_check(self):
        """
        Check if we are ready to provide a kernel.
        
        If we are, notify the dealer, unless it was done already.
        """
    ...
    def start(self):
        ...
        Process requests from the dealer and connection info from kernels.
        Prefork more standard kernels.
        
        Perhaps clean old kernel directories?!
        And even take care of killing old kernels ourselves if they were not
        killed by dealer messages?!
        """
        self.is_active = True
        poller = zmq.Poller()
